...
const listenDrag = () => {
    ...
    drag.onmousedown = function(event){
        var eventt = event || window.event;  //兼容IE浏览器
        //    鼠标点击物体那一刻相对于物体左侧边框的距离=点击时的位置相对于浏览器最左边的距离-物体左边框相对于浏览器最左边的距离
        var diffX = eventt.clientX - drag.offsetLeft;
        var diffY = eventt.clientY - drag.offsetTop;
        if(typeof drag.setCapture !== 'undefined'){
            drag.setCapture();
        }
        document.onmousemove = function(event){
            var mEvent = event || window.event;
            var moveX = mEvent.clientX - diffX;
            var moveY = mEvent.clientY - diffY;
            if(moveX < 0){
                moveX = 0;
